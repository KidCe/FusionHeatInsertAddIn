...
class SelectionDiagnosticsTests(unittest.TestCase):
    ...
    def test_invalidated_location_is_resolved_from_its_cached_entity_token(self):
        adsk = types.ModuleType("adsk")
        core = types.ModuleType("adsk.core")
        fusion = types.ModuleType("adsk.fusion")
        core.CommandEventHandler = _Handler
        ...
        design = types.SimpleNamespace(findEntityByToken=lambda token: [replacement])
        fusion.Design = types.SimpleNamespace(cast=lambda value: design)
        fusion.SketchPoint = types.SimpleNamespace(cast=lambda value: value)
        adsk.core = core
        adsk.fusion = fusion
        old_modules = {
            name: sys.modules.get(name) for name in ("adsk", "adsk.core", "adsk.fusion")
        ...
        try:
            ...
            selection = types.SimpleNamespace(
                selectionCount=1,
                selection=lambda index: types.SimpleNamespace(entity=invalid),
            )
            inputs = types.SimpleNamespace(itemById=lambda _input_id: selection)
            result = module._selected_points(
                inputs,
                selection_cache={
                    "locations:0": {"entity": invalid, "token": "point-token"}
                },
            )
